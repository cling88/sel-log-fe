...
export function SettingsView() {
  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      <section className="mt-6 rounded-2xl border border-black/15 bg-white p-6">
        <h2 className="text-base font-semibold text-black">수익 계산 설정</h2>
        <div className="mt-4 space-y-4">
          <label className="flex items-center justify-between text-sm">
            <span className="text-black/70">기본 마진율</span>
            <span className="flex items-center gap-1">
              <input
                type="number"
                value={margin}
                onChange={(e) => setMargin(Number(e.target.value) || 0)}
                className="h-9 w-20 rounded-md border border-black/15 px-2 text-right"
              />
              %
            </span>
          </label>
          <label className="flex items-center justify-between text-sm">
            <span className="text-black/70">네이버 수수료율</span>
            <span className="flex items-center gap-1">
              <input
                type="number"
                step="0.01"
        ...
        <div className="mt-5 rounded-xl bg-white p-4 text-sm text-black/70">
          <p className="font-medium text-black">권장판매가 계산식</p>
      ...
      <section className="mt-4 rounded-2xl border border-black/15 bg-white p-6">
        <h2 className="text-base font-semibold text-black">카테고리 관리</h2>
      ...
      <section className="mt-4 rounded-2xl border border-black/15 bg-white p-6">
        <h2 className="text-base font-semibold text-black">구매처 · 도매처 관리</h2>
        <p className="mt-1 text-xs text-black/60">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {vendors.map((vendor) => (
            <span
              key={vendor}
              className="inline-flex items-center gap-1 rounded-full border border-black/15 bg-white px-3 py-1 text-sm"
            >
              {editingVendor === vendor ? (
                <>
                  <input
                    ...
                    className="h-6 w-24 rounded border border-black/15 bg-white px-1 text-sm"
                  />
                  <button
                    type="button"
                    onClick={() => {
          ...
          <button
            ...
            className="rounded-full border border-dashed border-black/20 px-3 py-1 text-sm text-black/70 hover:bg-white"
          >
            + 추가
          </button>
        </div>
